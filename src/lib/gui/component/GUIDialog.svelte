	...
	>
		...
		<Dialog.Content forceMount>
			{#snippet child(context)}
				{#if context.open}
					<div
						class="max-w-md w-full sm:border-2 border-inactive-light dark:border-inactive rounded-2xl bg-input-light dark:bg-input sm:(bg-white dark:bg-black)"
						{...context.props}
						transition:fly={{ y: 120, duration: 300 }}
					>
						<header
							class="h-20 px-8 flex items-center justify-between border-b-2 border-inactive-light dark:border-inactive"
						>
							<Dialog.Title class="text-xl #text-summit font-bold" level={3}>
								{title}
							</Dialog.Title>
							<Dialog.Close class="button-square-input">
								<X class="#text-summit" size={24} />
							</Dialog.Close>
						</header>
