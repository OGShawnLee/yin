...
<div class="mt-2 flex items-center justify-between">
	<form action="/post/{post.id}?/handle-bookmark" method="post" use:enhance>
		...
		>
			...
			{#if post.bookmarkCount > 0}
				<p class="#text-summit font-black">
					{post.bookmarkCount}
				</p>
			{/if}
			<p class="hidden sm:block text-side">Bookmark Post</p>
		</button>
	</form>
	<form action="/post/{post.id}?/handle-favourite" method="post" use:enhance>
		...
		>
			...
			{/if}
			<p class="hidden sm:block text-side">Like Post</p>
		</button>
	</form>
